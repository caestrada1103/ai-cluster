...
class QueuedRequest:
    ...
    enqueued_at: float
    timeout: float
    future: "asyncio.Future[WorkerInfo]"
...
class RequestRouter:
    ...
    def __init__(
        ...
        self.queues: Dict[QueuePriority, "asyncio.Queue[QueuedRequest]"] = {
        ...
        self._load_updater_task: Optional["asyncio.Task[None]"] = None

        # Metrics
        self.routed_requests = metrics.counter(
            "router_routed_requests_total", "Total requests routed", ["strategy", "model"]
        )
        self.queue_size = metrics.gauge("router_queue_size", "Current queue size", ["priority"])
        self.circuit_breaker_open = metrics.gauge(
            "router_circuit_breaker_open", "Circuit breaker open count"
        )
            ...
            try:
                ...
                for worker_id, worker in workers.items():
                    ...
                    load = WorkerLoad(
                        ...
                        gpu_utilization=sum(g.utilization for g in worker.gpus) / len(worker.gpus)
                        if worker.gpus
                        else 0,
                        memory_used_gb=sum(g.total_memory - g.available_memory for g in worker.gpus)
                        / 1e9,
    ...
    def get_circuit_breaker_stats(self) -> Dict[str, Dict[str, Any]]:
        """Get circuit breaker statistics."""
        return {worker_id: cb.stats for worker_id, cb in self.circuit_breakers.items()}
