...
impl LlamaCppEngine {
    ...
    /// concurrent contexts at this engine's configured window. `slots` must
    /// be worst-case concurrency (each generation call opens a fresh
    /// `LlamaContext`), not `1` — the caller (`model_loader.rs`) passes
    /// `max_concurrent_requests`. Weights-independent; see
    /// [`crate::kv_estimate`] for the shared math and hybrid-attention notes.
